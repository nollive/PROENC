...
data <- read_excel(paste0(wd,"/",file_pheno_xlsx))
...
pd <- read.AnnotatedDataFrame(file = file_pheno, row.names = 1, header = TRUE, as.is = TRUE, sep = "\t", col.names = c("GSM", "group", "Patient", "Time", "group_short", "description", "file", "FileName"))
...
pheatmap(corMatrix,
         annotation_col=sampleInfo[, c("group_short", "Time")])    #-2 pour enlever colonne texte


# Supprimer les colonnes non nécessaires de sampleInfo
annotations <- sampleInfo[, c("group_short", "Time")]
...
```{r}
anno <- fData(data_filtr)
colnames(anno)


# A VERIFIER SI PAS DE PB (on a enlevé les genes NA et peu exprimés, on devrait réutiliser la table keep ??)

anno <- dplyr::select(features,GENE_SYMBOL,ENTREZID) #A CHANGER SELON LA TECHNOLOGIE 
...
```{r}
library(ggplot2)
ggplot(full_results,aes(x = logFC, y=2^B)) + geom_point()
```
...
full_results %>% 
  ...
  ggplot(aes(x = logFC, y = 2^B, col=Significant)) + geom_point()
```
...
full_results %>% 
  ...
  ggplot(aes(x = logFC, y = 2^B, col=Significant,label=Label)) + geom_point() + geom_text_repel(col="black")
```
...
full_results %>% 
  ...
  ggplot(aes(x = logFC, y = 2^B, col = Significant, label = Label)) + 
  geom_point() + 
