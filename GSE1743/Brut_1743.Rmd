...
annotations <- sampleInfo[, c("Group_short", "Group_short")]
...
data_outliers <- data_filtr[!data_filtr$geo_accession %in% GSM_outliers, ] #ne fonctionne pas pour l'instant
...
write_csv(full_output, path=paste0('GSE_',my_id,"_gse_full_output.csv"))
```
...
full_results <- full_results %>%
  mutate(gene_type = case_when(logFC >= log2(1.5) & adj.P.Val <= 0.05 ~ "up",
...
filter(full_results, adj.P.Val < 0.05, abs(logFC) > 1) %>%
  write_csv(path=paste0('GSE_',my_id,"_filtered_significant_results.csv"))
```
